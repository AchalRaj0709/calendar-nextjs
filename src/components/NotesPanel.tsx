...
export default function NotesPanel({
  ...
  // Helper formatter for the UI representing the date inside each note card
  const getNoteRangeLabel = (startDate: string, endDate: string) => {
    if (startDate === endDate) return null; // We don't render it if it's just a single day
    
    // Formatting them cleanly YYYY-MM-DD -> MMM D
  ...
  return (
    <motion.div
      ...
      id="notes-panel"
    >
      <div className="rounded-2xl bg-card-bg border border-card-border shadow-md overflow-hidden relative">
        ...
        <AnimatePresence>
          {toastMessage && (
            <motion.div
              ...
              exit={{ opacity: 0, y: -20 }}
              className="absolute top-2 right-2 left-2 z-50 bg-success text-white text-xs font-medium px-4 py-2 rounded-xl text-center shadow-md flex items-center justify-center gap-2"
            >
        ...
        <div className="max-h-[320px] overflow-y-auto">
          {notes.length === 0 ? (
            ...
            <AnimatePresence>
              {notes.map((note) => {
                const rangeLabel = getNoteRangeLabel(note.startDate, note.endDate);
                  ...
                  >
                    {editingId === note.id ? (
                      ...
                      <div className="p-4">
                        ...
                        <div className="flex gap-2 mt-2">
                          <button
                            ...
                            className="px-3 py-1.5 rounded-lg bg-accent text-white text-xs font-medium hover:bg-accent-hover transition-all active:scale-95 disabled:opacity-40"
                          >
                            Save
                          </button>
                          <button
                      ...
                      <div className="p-4 group">
                        {/* Range badge if applicable */}
                        {rangeLabel && (
                          <div className="mb-2">
                            <span className="inline-block px-2 py-0.5 rounded-md bg-accent/10 border border-accent/20 text-[10px] uppercase font-bold tracking-wider text-accent">
                              {rangeLabel}
                            </span>
                          </div>
                        )}
